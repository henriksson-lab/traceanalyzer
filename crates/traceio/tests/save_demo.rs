//! Round-trip tests for `traceio::save::save_run` against real demo fixtures.
...
fn round_trip_rename_from_gz_to_xml() {
    ...
    let mut run = traceio::bioanalyzer::parse_xml(&xml).expect("parse demo");
    ...
    traceio::save::save_run(&run, &src, &dst).expect("save_run");

    let reloaded_xml = std::fs::read_to_string(&dst).expect("read back");
    let reloaded = traceio::bioanalyzer::parse_xml(&reloaded_xml).expect("reparse");
...
fn saving_as_xad_is_rejected() {
    ...
    let run = traceio::model::Electrophoresis {
        assay: Default::default(),
        ladder_peaks: vec![],
        regions: vec![],
        samples: vec![],
    };
    let err = traceio::save::save_run(
        &run,
