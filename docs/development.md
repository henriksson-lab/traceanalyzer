...
cargo run --example inspect -- testdata/demo_dna1000.xml.gz
cargo run --example inspect -- testdata/demo_rna_nano.xml.gz
cargo run --example inspect -- testdata/tapestation/d1000.xml.gz
...
The `inspect` example uses `traceanalyzer::traceio::io::read_path`, which accepts Bioanalyzer
...
fn main() -> anyhow::Result<()> {
    let detected = traceanalyzer::traceio::io::detect_format("testdata/demo_dna1000.xml.gz")?
        .expect("supported electrophoresis file");
    println!("{:?}", detected.save_capabilities());

    let loaded = traceanalyzer::traceio::io::read_path("testdata/demo_dna1000.xml.gz")?;
    if traceanalyzer::traceio::io::supports_save_path(&loaded, "out.xml") {
        traceanalyzer::traceio::io::save_path(&loaded, "out.xml")?;
    }
...
The lower-level `traceanalyzer::traceio::save::save_run(&run, &src, &dst)` API is still
...
```sh
# Parser/analysis tests (validate against real demo files if fixtures exist):
cargo test

# GUI tests, including the headless render smoke test:
cargo test
```
...
- `traceanalyzer` uses Slint plus the `winit` backend. Running the GUI needs a
  ...
  `xvfb-run -a cargo run -- testdata/demo_dna1000.xml.gz`.
...
- `make deb` derives the Debian architecture from `dpkg --print-architecture`
  ...
  `DEB_RECOMMENDS`.
- Packaging can be checked with `cargo package`. The repository is a single
  package, so packaging no longer depends on publishing internal path crates
  first.
