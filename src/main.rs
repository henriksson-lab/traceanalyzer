...
//!
//! Usage: cargo run -- <file.xad | file.xml | file.xml.gz>
...
use traceanalyzer::traceio::calibration::{marker_times, MarkerOverride};
use traceanalyzer::traceio::Electrophoresis;
...
fn main() -> anyhow::Result<()> {
    ...
    // `--simulate <session.pck>` runs the no-hardware instrument simulator and
    // opens the resulting run in the viewer (milestone M5). Every other argument
    // is a file to open.
    let mut args: Vec<PathBuf> = Vec::new();
    let mut simulate: Vec<PathBuf> = Vec::new();
    let mut raw = std::env::args().skip(1);
    while let Some(a) = raw.next() {
        if a == "--simulate" {
            match raw.next() {
                Some(p) => simulate.push(PathBuf::from(p)),
                None => anyhow::bail!("--simulate needs a .pck path"),
            }
        } else {
            args.push(PathBuf::from(a));
        }
    }
    let paths: Vec<PathBuf> = if args.is_empty() && simulate.is_empty() {
    ...
    // Simulated instrument runs (`--simulate`): drive the recorded session
    // through the tracehw simulator and open the acquisition like any file.
    for pck in &simulate {
        match loading::simulate_pck(pck) {
            Ok(run) => {
                app.add_file(run, Vec::new(), Some(pck.clone()));
                app.set_active_opened_path(Some(pck.clone()));
                warnings.push(format!(
                    "Simulated instrument run from {} (provisional uncalibrated signal)",
                    pck.display()
                ));
            }
            Err(e) => warnings.push(format!("Could not simulate {}: {e:#}", pck.display())),
        }
    }
    ...
    // Simulate an instrument run from a recorded `.pck` session (milestone M5:
    // the no-hardware instrument path, reachable from the GUI).
    {
        let ui_weak = ui.as_weak();
        let st = state.clone();
        ui.on_simulate_run(move || {
            let Some(ui) = ui_weak.upgrade() else { return };
            if let Some(path) = rfd::FileDialog::new()
                .add_filter("Recorded instrument session", &["pck"])
                .pick_file()
            {
                simulate_added_file(&ui, &st, &path);
            }
        });
    }
...
/// Run the no-hardware instrument simulator over a recorded `.pck` and open the
/// resulting acquisition in the viewer (milestone M5).
fn simulate_added_file(ui: &AppWindow, state: &SharedState, path: &std::path::Path) {
    match loading::simulate_pck(path) {
        Ok(run) => {
            let table = {
                let mut s = state.borrow_mut();
                s.add_file(run, Vec::new(), Some(path.to_path_buf()));
                s.set_active_opened_path(Some(path.to_path_buf()));
                s.error = None;
        ...
        Err(e) => set_open_error(
            ui,
            state,
            format!("Could not simulate {}: {e:#}", path.display()),
        ),
    }
}
...
fn is_supported_open_path(path: &std::path::Path) -> bool {
    traceanalyzer::traceio::io::detect_format(path).is_ok_and(|detected| detected.is_some())
}
...
fn loaded_fa_metadata(
    path: &std::path::Path,
    metadata: Option<traceanalyzer::traceio::fa::FaMetadata>,
) -> Option<LoadedFaMetadata> {
    if !traceanalyzer::traceio::fa::is_fa_path(path) {
        return None;
    }
...
fn summarize_fa_metadata(
    metadata: &traceanalyzer::traceio::fa::FaMetadata,
) -> Vec<(String, String)> {
...
fn format_fa_metadata(metadata: &traceanalyzer::traceio::fa::FaMetadata) -> Option<String> {
...
fn write_ini_summary(
    out: &mut String,
    title: &str,
    document: &traceanalyzer::traceio::fa::IniDocument,
) {
...
fn sample_has_overview_trace_data(
    run: &Electrophoresis,
    sample: &traceanalyzer::traceio::Sample,
    y_mode: YMode,
) -> bool {
    series_has_points(&plot::series(run, sample, y_mode, false))
}

fn sample_has_gel_trace_data(sample: &traceanalyzer::traceio::Sample) -> bool {
...
fn sample_x_axis(st: &AppState, s: &traceanalyzer::traceio::Sample, overlay: bool) -> XAxis {
...
fn marker_lines(st: &AppState) -> Vec<f64> {
    ...
    let (lo, up) = traceanalyzer::traceio::calibration::marker_times(st.run(), idx, ov);
...
fn raw_channels_for_export(st: &AppState) -> Vec<&traceanalyzer::traceio::xad::RawChannel> {
...
fn sample_has_table_rows(
    run: &traceanalyzer::traceio::Electrophoresis,
    sample: &traceanalyzer::traceio::Sample,
) -> bool {
    ...
    let result = (|| -> anyhow::Result<SavedRun> {
        ...
        match s.source_path() {
            Some(src) => {
                ...
                traceanalyzer::traceio::save::save_run(s.run(), &src, &dst)?;
...
mod tests {
    use super::*;
    use traceanalyzer::traceio::{AssayInfo, Sample};
