...
use crate::traceio::calibration::{marker_times, MarkerOverride};
use crate::traceio::io::{DetectedFormat, SaveCapabilities, SourceCapabilities};
        ...
        self.active_file().is_some_and(|f| {
            ...
            !f.bioanalyzer_xad_mode()
                ...
                && (!f.fragment_analyzer_mode()
                    || f.source_path
                        .as_deref()
                        .is_some_and(crate::traceio::fa::has_saveable_txt_sidecar))
        })
    }
...
fn sample_has_processed_data(sample: &crate::traceio::Sample) -> bool {
...
mod tests {
    use super::*;
    use crate::traceio::io::TraceFormat;
    use crate::traceio::{AssayInfo, LadderPeak, Peak, Sample};
    ...
    fn tapestation_state_with_source(source: &str) -> AppState {
        ...
        run.samples[0].regions.push(crate::traceio::Region {
            lower_length: 200.0,
            upper_length: 400.0,
        });
        AppState::new(run, Vec::new(), Some(PathBuf::from(source)), None)
    ...
    fn raw_xad_can_save_as_when_detected_capabilities_allow_it() {
        let detected = crate::traceio::io::DetectedFormat {
    ...
    fn adopt_detected_source_updates_active_identity_and_capabilities() {
        let xad = crate::traceio::io::DetectedFormat {
            path: PathBuf::from("run.xad"),
            identity: PathBuf::from("run.xad"),
            format: TraceFormat::BioanalyzerXad,
            capabilities: SourceCapabilities {
        ...
        let xml = crate::traceio::io::DetectedFormat {
