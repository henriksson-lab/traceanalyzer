...
fn reads_fa_run_traces_and_names() {
    ...
    let run = traceanalyzer::traceio::fa::read_fa_run(&dir).expect("read FA run");
...
fn fa_peaks_and_markers_from_pks() {
    ...
    let ladders: Vec<&traceanalyzer::traceio::Sample> =
        run.samples.iter().filter(|s| s.is_ladder).collect();
...
fn path_api_reads_fa_metadata_from_fa_zip() {
    ...
    let loaded = traceanalyzer::traceio::io::read_path_with_metadata(&zip_path)
        .expect("path API reads FA zip");

    assert_eq!(loaded.loaded.source.identity, zip_path);
    assert!(matches!(
        loaded.loaded.source.format,
        traceanalyzer::traceio::io::TraceFormat::FragmentAnalyzerRun {
            entry: traceanalyzer::traceio::io::FragmentAnalyzerEntry::Zip
        }
    ));
...
fn path_api_saves_fa_zip_sidecar_names() {
    ...
    let mut loaded = traceanalyzer::traceio::io::read_path(&zip_path).expect("read path API");
    loaded.run.samples[0].name = "D1: public api rename".to_string();

    assert!(traceanalyzer::traceio::io::supports_save_path(
        &loaded, &zip_path
    ));
    traceanalyzer::traceio::io::save_path(&loaded, &zip_path).expect("save path API");

    let reloaded = traceanalyzer::traceio::io::read_path(&zip_path).expect("reload saved FA zip");
...
fn reads_fa_run_from_zip() {
    ...
    let from_dir = traceanalyzer::traceio::fa::read_fa_run(&dir).expect("read dir");
    let from_zip = traceanalyzer::traceio::fa::read_fa_run(&zip_path).expect("read zip");
    assert!(
        traceanalyzer::traceio::fa::is_fa_path(&zip_path),
...
fn saves_renamed_names_into_zip() {
    ...
    let mut run = traceanalyzer::traceio::fa::read_fa_run(&zip_path).expect("read zip");
    let orig_trace = run.samples[0].fluorescence.clone();
    run.samples[0].name = "D1: RENAMED_SAMPLE".to_string();
    traceanalyzer::traceio::fa::save_txt_names(&zip_path, &run).expect("save into zip");

    // Reload: the rename persists and the .raw-derived trace is untouched.
    let reloaded = traceanalyzer::traceio::fa::read_fa_run(&zip_path).expect("reload zip");
    assert!(
...
fn opens_from_any_run_member_file() {
    ...
    assert!(
        traceanalyzer::traceio::fa::is_fa_path(&pks),
        "a .PKS member should be FA"
    );
    assert!(
        traceanalyzer::traceio::fa::is_fa_path(&txt),
        "a .txt member should be FA"
    );
    assert!(
        traceanalyzer::traceio::fa::is_fa_path(&dir),
        "the run folder should be FA"
    );

    // ...and all map to one identity (the .raw), so a multi-file drop dedups.
    let id = traceanalyzer::traceio::fa::run_identity(&raw);
    assert_eq!(traceanalyzer::traceio::fa::run_identity(&pks), id);
    assert_eq!(traceanalyzer::traceio::fa::run_identity(&txt), id);
    assert_eq!(traceanalyzer::traceio::fa::run_identity(&dir), id);

    // Opening via a member decodes the whole run.
    let via_member = traceanalyzer::traceio::fa::read_fa_run(&pks).expect("open via .PKS");
    ...
    let hijacked = traceanalyzer::traceio::fa::is_fa_path(&stray_xml);
