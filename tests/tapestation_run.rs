...
fn d1000_xml() -> Option<PathBuf> {
    let p = PathBuf::from(env!("CARGO_MANIFEST_DIR")).join("testdata/tapestation/d1000.xml.gz");
...
fn reads_d1000_export_with_traces_and_sizing() {
    ...
    let run =
        traceanalyzer::traceio::tapestation::read_tapestation(&xml).expect("read D1000 export");
